...
% Compute the transformation of the e w.r.t. the robot base
bTe = geometricModel.getTransformWrtBase(length(jointType));
disp('bTe')
disp(bTe)
% Compute the transformation of the frame 3 w.r.t. the frame 5
...
%% Plot the initial and final configurations of the robot
figure
grid on
hold on
...
bri_initial = zeros(3, geometricModel.jointNumber + 1);
bri_initial(:, 1) = [0; 0; 0];  % Base position

for j = 1:geometricModel.jointNumber
    bTi(:, :, j) = geometricModel.getTransformWrtBase(j);
    bri_initial(:, j + 1) = bTi(1:3, 4, j);  % Extract translation component
end

plot3(bri_initial(1, :), bri_initial(2, :), bri_initial(3, :), 'bo-', 'LineWidth', 1.5, 'DisplayName', 'Initial Configuration');

% Plot the final configuration
geometricModel.updateDirectGeometry(qf);

bri_final = zeros(3, geometricModel.jointNumber + 1);
bri_final(:, 1) = [0; 0; 0];  % Base position

for j = 1:geometricModel.jointNumber
    bTi(:, :, j) = geometricModel.getTransformWrtBase(j);
    bri_final(:, j + 1) = bTi(1:3, 4, j);  % Extract translation component
end

plot3(bri_final(1, :), bri_final(2, :), bri_final(3, :), 'ro-', 'LineWidth', 1.5, 'DisplayName', 'Final Configuration');
